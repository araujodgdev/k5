...
- The mobile header already names the module next to the logo, so a module's own page title (Início, Lume, Cofre, Pesquisa, Agenda, Notificações, Administração) is `max-md:sr-only`: still the page's `h1`, not shown twice. Titles that name a record (a case, a client, a judgment) stay visible.
