...
function actor(): WorkspaceContext {
...
test('duas pessoas compartilham identidade pública e não histórico privado',async()=>{
  const installation=await source(),a=actor(),b=actor();
...
async function getResearchSearchAfterSourceId(id:string):Promise<string> {
  return String(testDb.prepare('SELECT source_judgment_id FROM research_judgment WHERE id=?').get(id)!.source_judgment_id);
}
...
test('FTS pesquisa além dos 400 mais recentes e pagina sem repetir IDs',async()=>{
  ...
  for (const [judgmentId,materialId,versionId,native,text,date,sourceId] of entries) {
    insertJudgment.run(judgmentId,sourceId,native,native,date,native);
    insertMaterial.run(materialId,judgmentId,versionId);
    insertVersion.run(versionId,materialId,versionId,text.length,text);
    insertFts.run(judgmentId,versionId,text);
  }
  const person=actor();
...
test('página externa persiste antes de exibir e replay de cursor não dispara nova página',async()=>{
  const installation=await source(),person=actor(),unique=`lookup-${randomUUID()}`;
...
test('repetir coleta mantém versão e IDs de trechos; alteração cria versão sem apagar a antiga',async()=>{
  ...
  const first=testDb.prepare(`SELECT v.id,m.id AS material_id FROM research_material m
    JOIN research_material_version v ON v.id=m.current_version_id WHERE m.judgment_id=? AND m.kind='ementa'`)
    .get(judgmentId)!;
  const firstChunk=testDb.prepare('SELECT id FROM research_chunk WHERE material_version_id=?').get(first.id)!.id;
  await upsertSourceJudgment(installation,value);
  assert.equal(testDb.prepare('SELECT id FROM research_chunk WHERE material_version_id=?').get(first.id)!.id,firstChunk);
  assert.equal(testDb.prepare('SELECT count(*) AS n FROM research_material_version WHERE material_id=?').get(first.material_id)!.n,1);
  await upsertSourceJudgment(installation,{...value,ementa:'Guarda e proteção especial revista'});
  const later=testDb.prepare('SELECT current_version_id FROM research_material WHERE id=?').get(first.material_id)!;
  assert.notEqual(later.current_version_id,first.id);
  assert.equal(testDb.prepare('SELECT id FROM research_chunk WHERE id=?').get(firstChunk)!.id,firstChunk);
  assert.equal(testDb.prepare('SELECT count(*) AS n FROM research_material_version WHERE material_id=?').get(first.material_id)!.n,2);
});

test('obtenção automática alcança só o material da página visitada e publica texto válido',async()=>{
  testDb.exec("UPDATE research_job SET status='cancelled' WHERE status IN ('queued','running')");
  const installation=await source(),person=actor(),id=`fetch-${randomUUID()}`;
  ...
  const material=testDb.prepare("SELECT id FROM research_material WHERE judgment_id=? AND kind='full_text'").get(judgmentId)!;
  assert.equal(testDb.prepare("SELECT count(*) AS n FROM research_job WHERE material_id=? AND status='queued'").get(material.id)!.n,1);
  const fixture=new Map([[fixtureKey(installation.id,'POST','/api/v1/pesquisa'),{
  ...
  const outcome=await processNextResearchExternalJob({transport:fixtureTransport(fixture)});
  assert.equal(outcome?.status,'completed');
  const version=testDb.prepare(`SELECT v.text_content FROM research_material m
...
test('reserva de quota é atômica e lease expirado após cinco tentativas termina',async()=>{
  testDb.exec("UPDATE research_job SET status='cancelled' WHERE status IN ('queued','running')");
  testDb.exec('DELETE FROM research_source_budget');
  const installation=await source(),person=actor();
  testDb.prepare('UPDATE judicial_source_installation SET rate_limit_per_minute=1 WHERE id=?').run(installation.id);
  ...
  const search=await startResearchSearch(person,{theme:`isolado${randomUUID().replaceAll('-','')}`,
    includeSources:true,idempotencyKey:randomUUID()});
  const job=testDb.prepare("SELECT id FROM research_job WHERE search_id=? AND kind='search_page'").get(search.id)!;
  testDb.prepare("UPDATE research_job SET status='running',attempts=5,lease_owner='dead',lease_until=1 WHERE id=?").run(job.id);
  await claimResearchJob('new-worker',['search_page']);
  assert.equal(testDb.prepare('SELECT status FROM research_job WHERE id=?').get(job.id)!.status,'failed');
...
test('lease roubado durante transporte impede publicação e não sobrescreve outro worker',async()=>{
  testDb.exec("UPDATE research_job SET status='cancelled' WHERE status IN ('queued','running')");
  const installation=await source(),person=actor(),theme=`lease${randomUUID().replaceAll('-','')}`;
  ...
  const transport=fixtureTransport(fixture,()=>{
    testDb.prepare("UPDATE research_job SET lease_owner='other',lease_until=? WHERE id=?").run(Date.now()+90_000,job.id);
  });
  const outcome=await processNextResearchExternalJob({transport,workerId:'first'});
  assert.equal(outcome?.status,'stale');
  assert.equal(testDb.prepare('SELECT count(*) AS n FROM research_judgment WHERE source_judgment_id=?').get(native)!.n,0);
  assert.equal(testDb.prepare('SELECT lease_owner FROM research_job WHERE id=?').get(job.id)!.lease_owner,'other');
});
...
test('PDF binário preserva bytes/hash e o worker publica texto extraído por página',async()=>{
  ...
  const version=testDb.prepare('SELECT sha256,storage_key,text_content,published_at FROM research_material_version WHERE id=?').get(versionId)!;
  ...
  assert.equal(testDb.prepare('SELECT reference FROM research_chunk WHERE material_version_id=?').get(versionId)!.reference,'página:1');
});

test('Worker Cloudflare recusa original antes do filesystem, mantendo leitura Node',async()=>{
  const installation=await source();
  ...
  const row=testDb.prepare("SELECT current_version_id FROM research_material WHERE judgment_id=? AND kind='full_text'")
  ...
  const storageKey=testDb.prepare('SELECT storage_key FROM research_material_version WHERE id=?')
  ...
  const unsupported=(error: unknown) => error instanceof ResearchError && error.code==='unsupported' &&
    /download do original ainda não está disponível/.test(error.message);
  try {
...
test('21 materiais locais pendentes enfileiram só os 20 exibidos; próxima ação obtém o 21º',async()=>{
  testDb.exec("UPDATE research_job SET status='cancelled' WHERE status IN ('queued','running')");
  const installation=await source(),person=actor(),token=`tema${randomUUID().replaceAll('-','')}`;
  ...
  assert.equal(testDb.prepare(`SELECT count(*) AS n FROM research_job WHERE search_id=? AND kind='fetch_material'
...
test('dois escritórios compartilham um download; parar um interesse não cancela o outro',async()=>{
  testDb.exec("UPDATE research_job SET status='cancelled' WHERE status IN ('queued','running')");
  const installation=await source(),a=actor(),b=actor(),token=`dedup${randomUUID().replaceAll('-','')}`;
  ...
  testDb.exec("UPDATE research_job SET run_after=0 WHERE status='queued' AND error_code='material_claimed'");
  ...
  assert.equal(testDb.prepare(`SELECT count(*) AS n FROM research_job WHERE search_id=? AND status='queued'`).get(two.id)!.n,1);
});

test('revogar documentos durante transporte impede publicação e retira inteiro teor do FTS',async()=>{
  testDb.exec("UPDATE research_job SET status='cancelled' WHERE status IN ('queued','running')");
  const installation=await source(),person=actor(),token=`revoga${randomUUID().replaceAll('-','')}`;
  ...
  testDb.prepare("UPDATE judicial_source_installation SET permission_documents='proibido' WHERE id=?").run(installation.id);
  ...
  await assert.rejects(publishMaterialText(installation,id,'full_text','versão publicada após revogação','text/plain'),
    /deixou de permitir/);
  assert.equal(testDb.prepare(`SELECT count(*) AS n FROM research_material_version v
    JOIN research_material m ON m.id=v.material_id WHERE m.judgment_id=? AND m.kind='full_text'`).get(id)!.n,1);
  testDb.prepare("UPDATE judicial_source_installation SET permission_documents='permitido' WHERE id=?").run(installation.id);
  ...
  const fixture=fixtureTransport(new Map([[fixtureKey(installation.id,'POST','/api/v1/pesquisa'),{
    body:JSON.stringify({hits:{value:1},registros:[{identificador:pending,ementa:`${pending} ementa`,
      possuiInteiroTeor:true,inteiroTeorHtml:'Conteúdo que não pode ser publicado'}]})}]]),()=>{
    testDb.prepare("UPDATE judicial_source_installation SET permission_documents='proibido' WHERE id=?").run(installation.id);
  });
  assert.equal((await processNextResearchExternalJob({transport:fixture}))?.status,'failed');
  assert.equal(testDb.prepare(`SELECT count(*) AS n FROM research_material_version v JOIN research_material m ON m.id=v.material_id
...
test('Retry-After não é encurtado e quota adia job sem gastar tentativas',async()=>{
  testDb.exec("UPDATE research_job SET status='cancelled' WHERE status IN ('queued','running')");
  testDb.exec('DELETE FROM research_source_budget');
  const installation=await source(),person=actor(),theme=`retry${randomUUID().replaceAll('-','')}`;
  const search=await startResearchSearch(person,{theme,includeSources:true,idempotencyKey:randomUUID()});
  ...
  const job=testDb.prepare("SELECT id,attempts,run_after FROM research_job WHERE search_id=? AND kind='search_page'").get(search.id)!;
  assert.equal(job.attempts,1);
  assert.ok(Number(job.run_after)>=before+1_800_000);
  testDb.prepare("UPDATE research_job SET run_after=0 WHERE id=?").run(job.id);
  testDb.prepare("UPDATE judicial_source_installation SET rate_limit_per_minute=1 WHERE id=?").run(installation.id);
  assert.equal((await processNextResearchExternalJob({transport}))?.status,'deferred');
  const after=testDb.prepare('SELECT attempts,status,run_after,error_code FROM research_job WHERE id=?').get(job.id)!;
...
test('falha entre armazenamento e banco deixa órfão removível sem tocar versões válidas',async()=>{
  ...
  testDb.exec(`CREATE TEMP TRIGGER research_fail_version BEFORE INSERT ON research_material_version
    BEGIN SELECT RAISE(FAIL,'injected_failure'); END`);
  try { await assert.rejects(stageResearchPdf(installation,id,bytes,'https://jurisdf.tjdft.jus.br/official.pdf'),/injected_failure/); }
  finally { testDb.exec('DROP TRIGGER research_fail_version'); }
...
test('lease vencido não pode ser renovado nem concluído pelo antigo dono',async()=>{
  ...
  assert.ok(claimed);
  testDb.prepare('UPDATE research_job SET lease_until=1 WHERE id=?').run(claimed.id);
  assert.equal(await renewResearchLease(claimed,'antigo'),false);
  await completeResearchJob(claimed,'antigo');
  assert.equal(testDb.prepare('SELECT status FROM research_job WHERE id=?').get(claimed.id)!.status,'running');
  const replacement=await claimResearchJob('novo',['search_page']);
  assert.equal(replacement?.id,claimed.id);
  await completeResearchJob(claimed,'antigo');
  assert.equal(testDb.prepare('SELECT lease_owner FROM research_job WHERE id=?').get(claimed.id)!.lease_owner,'novo');
});
...
test('limpeza conserva original STJ enquanto checkpoint de ingestão o utiliza',async()=>{
  ...
  testDb.prepare(`INSERT INTO research_source_resource
  ...
  testDb.prepare("UPDATE research_source_resource SET checkpoint=NULL,status='completed' WHERE id=?").run(resourceId);
  assert.ok(await sweepResearchOrphans(0)>=1);
  await assert.rejects(getResearchOriginal(key));
  assert.match((await getResearchOriginal(originalKey)).toString('binary'),/^PK/);
  testDb.prepare('UPDATE research_source_resource SET original_storage_key=NULL WHERE id=?').run(resourceId);
  assert.ok(await sweepResearchOrphans(0)>=1);
...
test('a página aplica rerank somente aos 20 exibidos e preserva os demais no acervo',async()=>{
  testDb.exec("UPDATE research_job SET status='cancelled' WHERE status IN ('queued','running')");
  const installation=await source(),person=actor();
  testDb.prepare("UPDATE judicial_source_installation SET permission_ai='permitido' WHERE id=?").run(installation.id);
  testDb.prepare('INSERT INTO platform_admin(user_id) VALUES(?)').run(person.userId);
  ...
  assert.equal(testDb.prepare(`SELECT count(*) AS n FROM research_job WHERE search_id=? AND kind='fetch_material'`)
    .get(search.id)!.n,20);
  assert.equal(testDb.prepare('SELECT count(*) AS n FROM research_rerank_cache WHERE office_id=? AND user_id=?')
...
test('consulta externa sem fonte temática apta mantém acervo local e sinaliza cobertura parcial',async()=>{
  const installation=await source(),person=actor(),theme=`semfonte${randomUUID().replaceAll('-','')}`;
  const judgmentId=await upsertSourceJudgment(installation,record(theme,`${theme} guarda da avó`));
  testDb.exec('UPDATE judicial_source_installation SET live_transport_enabled=0');
  const search=await startResearchSearch(person,{theme,includeSources:true,idempotencyKey:randomUUID()});
  ...
  assert.equal(testDb.prepare("SELECT count(*) AS n FROM research_job WHERE search_id=? AND kind='search_page'")
    .get(search.id)!.n,0);
});

test('replay reconstitui job de material após queda entre resultado e fila',async()=>{
  testDb.exec("UPDATE research_job SET status='cancelled' WHERE status IN ('queued','running')");
  const installation=await source(),person=actor(),native=`replay-${randomUUID()}`;
  ...
  testDb.prepare(`INSERT INTO research_search_result
  ...
  assert.equal(testDb.prepare('SELECT count(*) AS n FROM research_search_result WHERE search_id=? AND judgment_id=?')
    .get(search.id,judgmentId)!.n,1);
  assert.equal(testDb.prepare(`SELECT count(*) AS n FROM research_job WHERE search_id=? AND page_id=?
...
test('espelho STJ fica pesquisável sem agendar download individual impossível',async()=>{
  const person=actor(),theme=`espelho${randomUUID().replaceAll('-','')}`;
  const installation=await upsertInstallation({kind:'ckan',courtCode:'STJ',courtName:'Superior Tribunal de Justiça',
    degree:'superior',system:'not_applicable',purpose:'jurisprudence',baseUrl:'https://dadosabertos.web.stj.jus.br/',
    authKind:'none',discoveryStatus:'pilot',permissions:{query:'permitido',cache:'permitido',documents:'permitido',
      redistribution:'permitido',ai:'nao_esclarecido'},allowedHosts:['dadosabertos.web.stj.jus.br'],enabled:true,
  ...
  assert.equal(testDb.prepare("SELECT count(*) AS n FROM research_job WHERE material_id=? AND kind='fetch_material'")
    .get(material?.id)!.n,0);
});
