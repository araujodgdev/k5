...
function fixture(role = 'reviewer') {
  ...
  testDb.prepare('INSERT INTO user (id,email,name) VALUES (?,?,?)').run(userId, `${userId}@test.local`, 'Pessoa');
...
test('feedback hides names/metrics before voting, keeps order and allows reviewer personal feedback', async () => {
  ...
  const admin = fixture('administrator');
...
test('feedback concurrent duplicates preserve the first blind vote and never inflate counts', async () => {
  ...
  assert.equal((testDb.prepare('SELECT COUNT(*) AS n FROM model_feedback WHERE user_id=?').get(context.userId) as { n: number }).n, 1);
});
...
test('feedback isolates offices/users and rejects revoked memberships and spoofed fields', async () => {
  ...
  testDb.prepare('DELETE FROM office_member WHERE user_id = ?').run(a.userId);
...
test('dataset export requires platform access and consent; preferences are not approved SFT examples', async () => {
  const admin = fixture(), optedIn = fixture(), legacy = fixture();
  ...
  const rowId = (testDb.prepare('SELECT id FROM model_feedback WHERE user_id=?').get(optedIn.userId) as { id: string }).id;
...
test('ties remain ties and prior-round exposure is recorded in the dataset', async () => {
  ...
  testDb.prepare('INSERT INTO platform_admin (user_id) VALUES (?)').run(admin.userId);
  testDb.prepare(`INSERT INTO model_feedback (id,campaign_id,office_id,user_id,model_a,model_b,preference,assessment_a,assessment_b)
  ...
  const rowId = (testDb.prepare('SELECT id FROM model_feedback WHERE user_id=? AND campaign_id=?').get(context.userId, pilot.id) as { id: string }).id;
