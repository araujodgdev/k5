...
function fixture(role: WorkspaceContext['role'] = 'lawyer') {
  ...
  testDb.prepare('INSERT INTO user(id,email,name) VALUES(?,?,?)').run(userId, `${userId}@example.test`, 'Advogada');
  testDb.prepare('INSERT INTO office_member(id,office_id,user_id,role) VALUES(?,?,?,?)').run(randomUUID(), officeId, userId, role);
  testDb.prepare('INSERT INTO vault_case(id,office_id,name,created_by) VALUES(?,?,?,?)').run(caseId, officeId, 'Caso da cliente', userId);
  testDb.prepare(`INSERT INTO vault_document(id,office_id,case_id,scope,original_name,stored_name,mime_type,byte_size,sha256,status,created_by)
    VALUES(?,?,?,'case','relato.txt',?,'text/plain',100,?,'ready',?)`)
    .run(documentId, officeId, caseId, randomUUID(), randomUUID().replaceAll('-', ''), userId);
  testDb.prepare('INSERT INTO vault_document_chunk(id,document_id,office_id,ordinal,stable_reference,content) VALUES(?,?,?,?,?,?)')
...
function publicMaterial() {
  ...
  testDb.prepare(`INSERT INTO judicial_source_installation(id,kind,court_code,court_name,degree,system,purpose,auth_kind,discovery_status,
  ...
  testDb.prepare(`INSERT INTO research_judgment(id,installation_id,source_judgment_id,tribunal,title,metadata_hash,collected_at,status)
    VALUES(?,?,?,?,?,?,?,'active')`).run(judgmentId, installationId, randomUUID(), 'TJDFT', 'Guarda pela avó', 'abc', '2026-09-01');
  testDb.prepare(`INSERT INTO research_material(id,judgment_id,kind,status,current_version_id) VALUES(?,?,'ementa','ready',?)`)
    .run(materialId, judgmentId, versionId);
  const text = 'Acórdão sobre guarda da criança pela avó em situação de cuidado continuado.';
  testDb.prepare(`INSERT INTO research_material_version(id,material_id,sha256,mime_type,byte_size,text_content,parser_version,citation_metadata_json,
  ...
  testDb.prepare('INSERT INTO research_chunk(id,material_version_id,ordinal,text_content,reference) VALUES(?,?,0,?,?)')
    .run(chunkId, versionId, text, 'ementa:1');
  return { installationId, judgmentId, materialId, versionId, chunkId, text };
}

const profileInput = (f: ReturnType<typeof fixture>) => ({ caseId: f.caseId, expectedVersion: 0,
...
test('profile versions, per-fact evidence, reviewer and office isolation', async () => {
  const a = fixture(), b = fixture(), reviewer = fixture('reviewer');
  ...
  assert.equal(testDb.prepare('SELECT count(*) AS n FROM research_case_profile_revision WHERE case_id=?').get(a.caseId)!.n, 1);
});

test('queued assessment keeps stance apart from relevance and invalidates changed evidence', async () => {
  const a = fixture(), material = publicMaterial();
  await saveResearchCaseProfile(a.context, profileInput(a));
  testDb.prepare('INSERT INTO platform_admin(user_id) VALUES(?)').run(a.context.userId);
  ...
  testDb.prepare('UPDATE vault_document SET sha256=?,updated_at=CURRENT_TIMESTAMP WHERE id=?').run(randomUUID(), a.documentId);
...
test('explicit bypass, idempotent link, pinned historical citation and office boundaries', async () => {
  const a = fixture(), b = fixture(), material = publicMaterial();
  await saveResearchCaseProfile(a.context, profileInput(a));
  ...
  testDb.prepare("UPDATE research_judgment SET court_unit='Unidade nova',source_url='https://example.test/novo',title='Título novo' WHERE id=?")
  ...
  testDb.prepare("INSERT INTO ai_run(id,office_id,user_id,kind,input) VALUES(?,?,?,'draft','{}')")
    .run(runId, a.context.officeId, a.context.userId);
  testDb.prepare("INSERT INTO ai_artifact(id,office_id,user_id,run_id,title,content,source_refs) VALUES(?,?,?,?,'Minuta','Texto',?)")
  ...
  assert.equal(testDb.prepare('SELECT count(*) AS n FROM research_material_version WHERE id=?').get(material.versionId)!.n, 1);
});
...
test('reference update changes only its pinned version; exhausted leases terminate', async () => {
  ...
  testDb.prepare('UPDATE research_material SET current_version_id=? WHERE id=?').run(newVersionId, material.materialId);
  ...
  testDb.prepare(`INSERT INTO research_case_assessment(id,office_id,case_id,material_version_id,requested_by,input_fingerprint,status,mode,question_version,
  ...
  assert.equal(testDb.prepare('SELECT status FROM research_case_assessment WHERE id=?').get(exhaustedId)!.status, 'unavailable');
});
