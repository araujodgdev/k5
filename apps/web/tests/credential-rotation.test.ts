...
async function fixture() {
  ...
  await insert('typesafe_connection', { office_id: office, encrypted_api_key: cipher });
  await f.db.prepare('INSERT INTO typesafe_platform_connection(id,encrypted_api_key) VALUES(1,?) ON CONFLICT(id) DO UPDATE SET encrypted_api_key=EXCLUDED.encrypted_api_key').run(cipher);
  await insert('platform_secret_ref', { id: randomUUID(), user_id: actor, purpose: 'ai_connection_key', encrypted_secret: cipher, secret_hint: 'hidden', expires_at: Date.now() + 60_000 });
  ...
  await insert('push_subscription', { id: randomUUID(), office_id: office, user_id: actor, device_id: 'device-test', endpoint_hash: 'endpoint-test', encrypted_subscription: cipher, vapid_key_id: 'vapid-test', auth_generation: 1, subscribed_at: new Date().toISOString(), last_reconciled_at: new Date().toISOString() });
  await insert('google_connection', { id: connection, office_id: office, user_id: actor, google_subject: 'synthetic', email: 'google@example.test', status: 'active', encrypted_refresh_token: cipher, encrypted_access_token: cipher });
  await f.db.prepare("INSERT INTO google_oauth_state(id,state_hash,office_id,user_id,session_id,encrypted_verifier,modules,scopes,expires_at) VALUES(?,?,?,?,?,?,ARRAY['gmail'],ARRAY['email'],CURRENT_TIMESTAMP+INTERVAL '1 minute')")
    .run(randomUUID(), randomUUID(), office, actor, randomUUID(), cipher);
  await insert('google_operation', { id: randomUUID(), office_id: office, user_id: actor, connection_id: connection, action: 'test', capability_name: 'test', invocation: 'ui', idempotency_key: randomUUID(), request_hash: 'synthetic', encrypted_args: cipher, encrypted_result: cipher, checkpoint_json: cipher, policy_version: 1, policy_mode: 'automatic', status: 'pending', usage_day: '2026-09-24' });
  return { ...f, old, next, ring, actor, secret, cipher };
}
...
test('rotation covers every encrypted column atomically, preserves empty references and is idempotent', async () => {
  const { db, pool, ring, actor, next, secret, cipher } = await fixture();
  ...
  const updated = await db.prepare('SELECT encrypted_api_key FROM ai_connection').get<{ encrypted_api_key: string }>();
