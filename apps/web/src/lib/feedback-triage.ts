...
import { ticketKinds, ticketModules, type TicketKind, type TicketModule, type TicketPriority } from './feedback-tickets-contract';
import type { TriageRecord } from './feedback-tickets';

export const feedbackTriageQuestionVersion = 'feedback-triage-pt-BR-v1';
const MAX_ATTEMPTS = 3;

/** Starting thresholds from the TypeSafe confidence-routing guidance; calibrate on real tickets. */
export const triageThresholds = { review: 0.6, flag: 0.5, urgentSeverity: 2.5, highSeverity: 1.5 };
...
export function triageQuestions(): Questions {
  return {
    kind: { type: 'choice', instructions: 'Que tipo de retorno a pessoa deixou em `feedback.message`?', criteria: kindCriteria },
    module: { type: 'choice', instructions: 'Qual parte do produto o texto em `feedback.message` trata? `feedback.page` é a tela em que a pessoa estava ao enviar: use como pista, mas o texto prevalece.', criteria: moduleCriteria },
...
export type Triage = {
  kind: TicketKind | null; module: TicketModule | null; severity: number | null; priority: TicketPriority;
...
export function composeTriage(response: DecisionResponse | undefined): Triage {
  ...
  const priority: TicketPriority = securityFlag || (severity ?? 0) >= triageThresholds.urgentSeverity ? 'p0'
    : (severity ?? 0) >= triageThresholds.highSeverity ? 'p1'
      : kind?.value === 'problem' || kind?.value === 'suggestion' ? 'p2' : kind ? 'p3' : 'p2';
  return {
    kind: kind?.value ?? null, module: area?.value ?? null, severity, priority, securityFlag,
...
type Claimed = { id: string; office_id: string; user_id: string | null; message: string; page_path: string; attempts: number; version: number };
...
export async function processNextFeedbackClassification(options: { send?: DecisionTransport } = {}): Promise<boolean> {
  ...
  const row = await database.prepare(`UPDATE feedback_ticket SET classification_status='running',lease_token=?,lease_until=?,attempts=attempts+1
    ...
    RETURNING id,office_id,user_id,message,page_path,attempts,version`).get<Claimed>(token, now + 30_000, now);
  if (!row) return false;
  const evaluated = await evaluate({ officeId: row.office_id, userId: row.user_id }, 'feedback', {
    state: { feedback: { message: row.message, page: row.page_path || 'desconhecida' } },
  ...
  await database.batch([
    database.prepare(`UPDATE feedback_ticket SET classification_status=?,classification_json=?,lease_token=NULL,lease_until=0,
        ...
        severity=?, security_flag=?, personal_data_flag=?,
      ...
      triage?.severity ?? null, triage?.securityFlag ?? false, triage?.personalDataFlag ?? false, triage ? triage.needsReview : true, classified, row.id, token),
