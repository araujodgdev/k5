...
export type ConnectionRow = {
  ...
  rag_mode: DecisionMode; documents_mode: DecisionMode; agenda_mode: DecisionMode; research_mode: DecisionMode; feedback_mode: DecisionMode;
...
export async function connectionView(db: Database = database): Promise<ConnectionView> {
  ...
  return row ? { model: row.model, enabled: Boolean(row.enabled), rag: row.rag_mode, documents: row.documents_mode,
    agenda: row.agenda_mode, research: row.research_mode, feedback: row.feedback_mode, dailyTokens: row.daily_tokens, concurrency: row.concurrency, version: row.version,
...
export async function saveConnection(actor: string, raw: ConnectionSettings, db: Database = database) {
  ...
  const result = await db.prepare(`INSERT INTO typesafe_platform_connection(id,encrypted_api_key,key_hint,model,enabled,rag_mode,documents_mode,agenda_mode,research_mode,feedback_mode,daily_tokens,concurrency)
    ...
    feedback_mode=excluded.feedback_mode,daily_tokens=excluded.daily_tokens,concurrency=excluded.concurrency,version=typesafe_platform_connection.version+1,failures=0,circuit_until=0,updated_at=CURRENT_TIMESTAMP
    WHERE typesafe_platform_connection.version=?`).run(secret, hint, input.model, Number(input.enabled), input.rag, input.documents, input.agenda, input.research, input.feedback, input.dailyTokens, input.concurrency, input.version);
  ...
  await db.prepare('INSERT INTO platform_audit_log(id,actor_user_id,office_id,action,details_json) VALUES(?,?,?,?,?)')
    .run(randomUUID(), actor, null, 'typesafe.configured', JSON.stringify({ model: input.model, enabled: input.enabled, rag: input.rag, documents: input.documents, agenda: input.agenda, research: input.research, feedback: input.feedback }));
