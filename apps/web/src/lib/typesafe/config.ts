...
export type ConnectionRow = {
  ...
  rag_mode: DecisionMode; documents_mode: DecisionMode; agenda_mode: DecisionMode;
...
export async function connectionView(officeId: string, db = database): Promise<ConnectionView> {
  ...
  return row ? { model: row.model, enabled: Boolean(row.enabled), rag: row.rag_mode, documents: row.documents_mode,
    agenda: row.agenda_mode, dailyTokens: row.daily_tokens, concurrency: row.concurrency, version: row.version,
...
export async function saveConnection(officeId: string, actor: string, raw: ConnectionSettings, db: Database = database) {
  ...
  const result = await db.prepare(`INSERT INTO typesafe_connection(office_id,encrypted_api_key,key_hint,model,enabled,rag_mode,documents_mode,agenda_mode,daily_tokens,concurrency)
    VALUES(?,?,?,?,?,?,?,?,?,?) ON CONFLICT(office_id) DO UPDATE SET encrypted_api_key=excluded.encrypted_api_key,key_hint=excluded.key_hint,
    model=excluded.model,enabled=excluded.enabled,rag_mode=excluded.rag_mode,documents_mode=excluded.documents_mode,agenda_mode=excluded.agenda_mode,
    ...
    WHERE typesafe_connection.version=?`).run(officeId, secret, hint, input.model, Number(input.enabled), input.rag, input.documents, input.agenda, input.dailyTokens, input.concurrency, input.version);
  ...
  await db.prepare('INSERT INTO platform_audit_log(id,actor_user_id,office_id,action,details_json) VALUES(?,?,?,?,?)')
    .run(randomUUID(), actor, officeId, 'typesafe.configured', JSON.stringify({ model: input.model, enabled: input.enabled, rag: input.rag, documents: input.documents, agenda: input.agenda }));
