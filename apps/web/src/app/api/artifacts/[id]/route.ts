...
export async function PUT(request: Request, context: Context) {
  try {
    ...
    if (!ownedArtifact(database, owner, id)) throw new ApiError(404, 'Documento não encontrado.');
    const updated = updateArtifact(database, owner, id, body.title, body.content, body.version);
