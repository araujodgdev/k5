...
import { archiveNotification } from '@/lib/application/notifications-service';
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    return noStore(Response.json({ updated: await archiveNotification(workspaceContext(workspace), id) }));
