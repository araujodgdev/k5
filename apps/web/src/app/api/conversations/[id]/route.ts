...
import { forgetThread } from '@/lib/agent-memory';
type Context = { params: Promise<{ id: string }> };
export async function GET(request: Request, context: Context) {
  try {
    const { user, office } = await apiWorkspace(request);
    const result = await conversation(database, { officeId: office.officeId, userId: user.id }, (await context.params).id);
...
export async function DELETE(request: Request, context: Context) {
  try {
    ...
    await forgetThread({ officeId: office.officeId, userId: user.id }, id);
