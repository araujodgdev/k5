...
export function TypesafeSettings({ officeId, initial }: { officeId: string; initial: ConnectionView }) {
  ...
  async function submit(method: string) {
    ...
    try {
      const response = await fetch(`/api/platform/offices/${encodeURIComponent(officeId)}/typesafe`, {
  ...
  return <section className="mt-8 border-t py-7" aria-labelledby="typesafe-title">
    <h2 id="typesafe-title" className="font-medium">TypeSafe</h2>
    <p className="mt-2 text-sm text-muted-foreground">Relevância de fontes, verificação documental e sugestões de agenda deste escritório.</p>
