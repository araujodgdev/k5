...
export function AgendaWorkspace({ role, initialCaseId, initialClientId, initialActivityId, initialProposalId = '', initialView = 'tasks', initialAction = '' }: {
  ...
  return <div className="flex min-w-0 flex-1 flex-col px-4 py-6 md:px-8 md:py-8 [&_[data-slot=button]]:min-h-11 md:[&_[data-slot=button]]:min-h-9">
    <header className="flex flex-wrap items-center justify-between gap-4 border-b pb-5"><h1 className="display text-[28px] max-md:sr-only">Tarefas e Agenda</h1><div className="flex gap-2"><Button variant="ghost" onClick={refresh} disabled={loading}>Atualizar</Button>{canWrite && <Button disabled={!optionsReady} className="h-11 md:h-9" onClick={() => setEditor({ mode: view === 'clients' ? 'client' : 'activity' })}>{view === 'clients' ? 'Novo cliente' : 'Nova atividade'}</Button>}</div></header>
