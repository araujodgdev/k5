...
import { agendaCall, localDate } from './agenda-forms';
...
export function CommandCenter({ role, name }: { role: OfficeRole; name: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const [tasks, meetings, clients, vault, conversations] = await Promise.allSettled([
        agendaCall('k5_agenda_list_activities', { kind: 'task', status: 'pending', dueTo: day, limit: 5 }),
        agendaCall('k5_agenda_list_activities', { kind: 'meeting', status: 'pending', from: now.toISOString(), limit: 4 }),
        agendaCall('k5_crm_list_clients', { stage: 'active', limit: 4 }),
        agendaCall('k5_vault_list_cases', {}),
        agendaCall('k5_conversations_list', { limit: 3 }),
      ]);
      if (cancelled) return;
      setToday(day);
      setData({ tasks: tasks.status === 'fulfilled' ? tasks.value : undefined, meetings: meetings.status === 'fulfilled' ? meetings.value : undefined, clients: clients.status === 'fulfilled' ? clients.value : undefined, vault: vault.status === 'fulfilled' ? vault.value : undefined, conversations: conversations.status === 'fulfilled' ? conversations.value : undefined });
      setLoading(false);
    }
  ...
  return <div className="min-w-0 flex-1 overflow-y-auto px-5 py-6 md:px-8 md:py-8">
    ...
    <div className="my-7 grid grid-cols-2 gap-x-6 gap-y-5 border-y py-5 sm:grid-cols-4" aria-label="Resumo do escritório">
      ...
      ].map(([label, count, href]) => <Link key={label} href={String(href)} className="rounded-sm outline-none focus-visible:ring-2 focus-visible:ring-ring"><span className="text-xs text-muted-foreground">{label}</span><span className="mt-2 block text-2xl tabular-nums">{loading || count === undefined ? '—' : count}</span></Link>)}
    </div>
    {failure && <p role="alert" className="mb-4 text-sm text-destructive">{failure}</p>}
    <div className="grid gap-x-10 gap-y-8 lg:grid-cols-2">
      <OverviewSection title="Tarefas até hoje" href="/app/agenda" loading={loading} failed={!data.tasks}>
      ...
      <OverviewSection title="Próximas reuniões" href="/app/agenda?view=calendar" loading={loading} failed={!data.meetings}>
        {data.meetings?.activities.length ? <div className="divide-y">{data.meetings.activities.map(activity => <Link key={activity.id} href={`/app/agenda?view=calendar&activityId=${encodeURIComponent(activity.id)}`} className="flex items-start gap-4 py-4 underline-offset-4 hover:underline"><time dateTime={activity.startsAt!} className="w-16 shrink-0 text-sm tabular-nums text-schedule">{new Date(activity.startsAt!).toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</time><div className="min-w-0"><p className="break-words text-sm">{activity.title}</p><p className="mt-1 text-[13px] text-muted-foreground">{new Date(activity.startsAt!).toLocaleDateString('pt-BR', { day: 'numeric', month: 'long' })}</p></div></Link>)}</div> : empty('Nenhuma reunião agendada.')}
      </OverviewSection>
      <OverviewSection title="Casos no Cofre" href="/app/vault" loading={loading} failed={!data.vault}>
      ...
      <OverviewSection title="Clientes ativos" href="/app/agenda?view=clients" loading={loading} failed={!data.clients}>
      ...
      <div className="lg:col-span-2"><OverviewSection title="Conversas com Lume" href="/app/agents" loading={loading} failed={!data.conversations}>
