...
export function AgendaEditor({ activity, client, mode, cases, clients, members, day, caseId, clientId, timeZone, close, saved }: {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (mode === 'client') {
        ...
        await agendaCall(activity ? 'k5_agenda_update_activity' : 'k5_agenda_create_activity', {
          ...
          startsAt: kind === 'meeting' ? new Date(text('startsAt')).toISOString() : null,
          endsAt: kind === 'meeting' ? new Date(text('endsAt')).toISOString() : null,
  ...
  return <Dialog open onOpenChange={open => { if (!open && !busy) close(); }}><DialogContent showCloseButton={false} overlayClassName="bg-foreground/25 supports-backdrop-filter:backdrop-blur-none" className="max-h-[85dvh] overflow-y-auto sm:max-w-xl [&_[data-slot=button]]:min-h-11 md:[&_[data-slot=button]]:min-h-9">
    <DialogHeader><DialogTitle>{mode === 'client' ? (client ? 'Editar cliente' : 'Novo cliente') : (activity ? 'Editar atividade' : 'Nova atividade')}</DialogTitle><DialogDescription className="sr-only">Preencha os dados e salve as alterações.</DialogDescription></DialogHeader>
    <form onSubmit={submit} onChange={() => setKey(crypto.randomUUID())} className="grid gap-4">
      ...
      <div className="flex justify-end gap-2"><Button type="button" variant="outline" disabled={busy} onClick={close}>Cancelar</Button><Button type="submit" disabled={busy}>{busy ? 'Salvando…' : 'Salvar'}</Button></div>
    </form>
