// Opt-in network evaluation. Uses an in-memory database and synthetic data only.
...
testDb.prepare('INSERT INTO office(id,name) VALUES(?,?)').run(officeId, 'Avaliação sintética');
testDb.prepare('INSERT INTO user(id,email,name) VALUES(?,?,?)').run(userId, `${userId}@example.test`, 'Avaliador');
...
function budget() {
  const used = Number(testDb.prepare('SELECT coalesce(sum(input_tokens),0) AS n FROM typesafe_evaluation').get()!.n);
  if (used > 1_000_000) throw new Error('Orçamento da avaliação atingido.');
}
for (const sample of ragCorpus.slice(0, limit)) {
  budget(); const ids: string[] = []; const grades = new Map<string, number>();
  for (const [n, item] of sample.texts.entries()) {
    ...
    testDb.prepare("INSERT INTO vault_document(id,office_id,scope,original_name,stored_name,mime_type,byte_size,sha256,status,created_by) VALUES(?,?,'library',?,?,'text/plain',100,?,'ready',?)").run(id, officeId, `fonte-${n}.txt`, id, id, userId);
    testDb.prepare("INSERT INTO vault_document_chunk(id,document_id,office_id,ordinal,stable_reference,content) VALUES(?,?,?,0,'parágrafo:1',?)").run(chunk, id, officeId, item.text);
  }
  const input = { query: sample.query, documentIds: ids, limit: 8 };
  testDb.prepare("UPDATE typesafe_connection SET rag_mode='off' WHERE office_id=?").run(officeId);
  const baseline = await searchKnowledgeEngine(context, input);
  testDb.prepare("UPDATE typesafe_connection SET rag_mode='enabled' WHERE office_id=?").run(officeId);
...
for (let i = 0; i < Math.min(documentCorpus.length, limit); i += 4) {
  budget(); const samples = documentCorpus.slice(i, Math.min(i + 4, limit));
...
for (const sample of agendaCorpus.slice(0, limit)) {
  budget(); const start = performance.now(); const result = await interpretAgenda(context, { message: sample.message, timeZone: 'America/Sao_Paulo' });
...
const usage = testDb.prepare('SELECT count(*) AS calls,coalesce(sum(input_tokens),0) AS inputTokens,coalesce(sum(output_tokens),0) AS outputTokens FROM typesafe_evaluation').get();
...
main().catch(() => { console.error('A avaliação falhou. Confira a configuração local; detalhes do provider foram ocultados.'); process.exitCode = 1; });
