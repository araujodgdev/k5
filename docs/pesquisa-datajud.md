...
O cadastro inicial de [casos do Cofre](../apps/web/db/migrations/0003_vault.sql) não modela número CNJ. A proposta é criar vínculos processuais separados, permitindo mais de um processo/registro por caso. O [catálogo de ferramentas](../apps/web/src/lib/agent-tools/index.ts) é um ponto de integração; o [fluxo de documentos](../apps/web/src/lib/document-workflows.ts) já trabalha com fontes e referências estáveis.
