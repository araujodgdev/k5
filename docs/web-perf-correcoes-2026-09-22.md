...
O build manteve os avisos conhecidos de tracing de caminho dinâmico em `storage/index.ts`, link de `mammoth` no Windows e aviso experimental de localStorage do Node. Não impediram a compilação. As medições abaixo cobrem o build Next.js local e não representam medição de usuários reais.

**Preparação da publicação em staging.** A verificação do Worker anterior às correções confirmou HTTP 500 nas quatro páginas autenticadas, enquanto entrada e login respondiam HTTP 200. Os logs identificaram um timer iniciado por `gsap.registerPlugin(useGSAP)` durante o import de módulos SSR. [AppSidebar](../apps/web/src/components/app-sidebar.tsx) e [Reveal](../apps/web/src/components/reveal.tsx) agora registram o plugin apenas no navegador. Dois [testes de regressão](../apps/web/tests/animation-ssr.test.ts) importam os componentes reais em processos novos com timers proibidos; ambos falharam antes da correção e passaram depois. Lint, typecheck, os **222 testes** e o build Next.js passaram novamente.

O primeiro CI da publicação também expôs a ausência do setup do SQLite: a coleta paralela de páginas encontrava tabelas ausentes e `database is locked`. O [workflow](../.github/workflows/ci.yml) passou a executar `pnpm db:setup` antes de `pnpm build`, com um arquivo SQLite exclusivo do CI e credenciais de compilação. A publicação da Cloudflare continua usando o script existente, que verifica as migrações remotas do D1 antes de enviar o Worker.

A verificação posterior confirmou HTTP 200 nas páginas autenticadas e identificou uma diferença no histórico: o mesmo horário aparecia como `2 h` no HTML do Worker e `Agora` no navegador em São Paulo. O SQLite grava `CURRENT_TIMESTAMP` em UTC sem incluir o fuso na string. O [formatador do histórico](../apps/web/src/lib/conversation-time.ts) agora interpreta esse formato explicitamente como UTC, preservando timestamps ISO com offset. Somente o texto relativo tolera a passagem de um minuto durante a hidratação. Os [testes de horário](../apps/web/tests/conversation-time.test.ts) fixam um fuso diferente de UTC e reproduziram a diferença de três horas antes da correção. A suíte passou com **224 testes**, além de lint e typecheck.
